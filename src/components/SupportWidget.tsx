      ...
      {open && (
        ...
        >
            <div className="flex items-center justify-between border-b border-slate-200 px-5 py-4">
              <div><h2 id="support-dialog-title" className="text-sm font-bold text-slate-900">Buykori Support</h2><p className="text-xs text-slate-500">Share the problem with screenshots or documents.</p></div>
              <Button variant="icon" size="lg" onClick={() => setOpen(false)} aria-label="Close support dialog"><X className="h-4 w-4" /></Button>
            </div>
            <div className="overflow-y-auto p-5">
              <form onSubmit={submit} className="space-y-3">
                <input ref={subjectInputRef} required maxLength={160} value={subject} onChange={e => setSubject(e.target.value)} placeholder="What do you need help with?" className="w-full rounded-lg border border-slate-200 px-3 py-2 text-xs" />
                ...
                <p className="text-xs text-slate-400">Up to 3 files, 2 MB each and 4 MB total.</p>
                {files.length > 0 && <p className="text-xs text-slate-500">{files.map(file => file.name).join(', ')} &middot; {files.length} selected</p>}
                <Button type="submit" variant="primary" size="md" loading={submitting} className="w-full text-xs">{submitting ? <Loader2 className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />} Submit request</Button>
              </form>
              <div className="mt-6 border-t border-slate-100 pt-4">
                <div className="flex items-center justify-between">
                  <h3 className="text-xs font-bold uppercase tracking-wider text-slate-500">Recent requests</h3>
                  <button type="button" onClick={refreshTickets} disabled={refreshing} className="inline-flex items-center gap-1 rounded-md px-2 py-1 text-xs font-bold text-indigo-600 hover:bg-indigo-50 disabled:opacity-60"><RefreshCw className={`h-3 w-3 ${refreshing ? 'animate-spin' : ''}`} /> Refresh</button>
                </div>
