...
export function PlanBillingSection({
  ...
  const currentPrice = isFree
    ? 'Free'
    : isStarter
      ? profile.isTrial ? '14-day Starter trial' : 'BDT 499 / month'
      : isGrowth
        ? 'BDT 799 / month'
        : isPro
          ? 'Custom billing'
      ...
      <section className="grid gap-5 rounded-xl border border-slate-200 bg-white p-5 shadow-sm lg:grid-cols-2 lg:items-center">
        <div>
          ...
          <div className="mt-1 flex flex-wrap items-center gap-2">
            ...
            <span className="inline-flex items-center gap-1 rounded-full border border-emerald-200 bg-emerald-50 px-2 py-0.5 text-[10px] font-bold text-emerald-700"><Check className="h-3 w-3" /> {profile.isTrial ? `14-day trial · ${trialDays} days left` : 'Active'}</span>
          </div>
        ...
        {plans.map(plan => (
          <section key={plan.label} className={`flex flex-col rounded-xl border bg-white p-5 shadow-sm ${plan.recommended ? 'border-indigo-500' : 'border-slate-200'}`}>
            <div className="flex flex-wrap items-center gap-2">
              ...
              {plan.active && plan.tier === 'starter' && profile.isTrial && <span className="rounded-full border border-amber-200 bg-amber-50 px-2 py-0.5 text-[10px] font-bold text-amber-700">14-day trial · {trialDays} days left</span>}
            ...
            {plan.contact ? (
              ...
              <button
                type="button"
                disabled={plan.active || isAgency || (plan.tier === 'starter' && currentRank > 1)}
                onClick={() => openPayment(plan.tier)}
                className={`mt-6 min-h-10 w-full rounded-lg px-4 text-xs font-bold transition ${plan.active || (plan.tier === 'starter' && currentRank > 1) ? 'bg-slate-100 text-slate-400' : 'bg-indigo-600 text-white hover:bg-indigo-700'} disabled:cursor-not-allowed`}
              >
                {plan.active
                  ? 'Your current plan'
                  : isAgency
                    ? 'Managed by support'
                    : plan.tier === 'starter' && currentRank > 1
                      ? 'Included in current plan'
                      : isStarter && plan.tier === 'growth'
                        ? 'Upgrade your plan'
                        : `Choose ${plan.label}`}
              </button>
            ) : (
